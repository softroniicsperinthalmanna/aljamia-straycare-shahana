...
if(isset($_POST['submit'])) {
    if (isset($_POST['sname']) && isset($_POST['sid']) && isset($_POST['password'])) {
        ...
        if ($con->connect_error) {
            ...
            $sql1 = mysqli_query($con, "INSERT INTO login (code,password,authority_type) values('$s_id','$pass','Forest')");
            $log_id = mysqli_insert_id($con);
            $sql2 = mysqli_query($con, "INSERT INTO station (station_name,login_id) values ('$s_name','$log_id')");
            if ($sql1 && $sql2) {
                echo "User added successfully";
                echo "<script>window.location.href='forest-table.php'; </script>";
            } else {
                echo "Fail to add details !!";
                header('location:forest-insert.php');
            }
        }
    }
}
